...
# -*- coding: utf-8 -*-
"""
导出 PMML 模型 - 使用 nyoka（纯 Python，不需要 Java）

使用方法：
    pip install nyoka
    python scripts/export_pmml_nyoka.py
...
from nyoka import skl_to_pmml  # nyoka 5.x 正确的导入方式
...
def main():
    ...
    try:
        skl_to_pmml(
            ...
            target_name='is_ai',
            pmml_f_name=output_path
        )
        print("   导出成功！")
    except Exception as e:
        print(f"   ❌ 导出失败: {e}")
        print("   提示：请确保已安装 nyoka:")
